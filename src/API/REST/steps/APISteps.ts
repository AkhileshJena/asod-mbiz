...
export default class APISteps {
    ...
    private GET_BASE_URL = process.env.REST_API_BASE_URL_GET_MOBILITY_ORDS;
    constructor(private page: Page) {
        this.api = new APIActions(this.page);
    }
    private get header() {
        return this.api.header.set(RESTConstants.CONTENT_TYPE, RESTConstants.CONTENT_JSON)
            .set(RESTConstants.ACCEPT, RESTConstants.CONTENT_JSON).set(RESTConstants.BASIC_AUTH,RESTConstants.BEARER_AUTH).get();
    }
    private get getHeader(){
        return this.api.header.set(RESTConstants.CONTENT_TYPE, RESTConstants.CONTENT_JSON)
            .set(RESTConstants.ACCEPT, RESTConstants.CONTENT_JSON).set(RESTConstants.BASIC_AUTH,RESTConstants.BEARE_AUTH_GET).get();
    }
    ...
    public async get(endPoint: string, operation: string): Promise<RESTResponse> {
        ...
        await test.step(`Making call to GET ${operation}`, async () => {
            response = await this.api.rest.get(this.GET_BASE_URL + endPoint, this.getHeader, operation);
        });        
        return response;
    }
